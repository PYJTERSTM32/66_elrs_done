...
void ICACHE_RAM_ATTR LinkStatsFromOta(OTA_LinkStats_s * const ls)
{
  ...
  // Publish telemetry to EXTREME-BLE (1Hz rate)
...
void ICACHE_RAM_ATTR SendRCdataToRF()
{
  ...
  else
  {
    ...
    else
    {
      ...
      #if defined(Regulatory_Domain_EU_CE_2400)
      static uint32_t last_ble_rc_update = 0;
      if (now - last_ble_rc_update > 20000) { // 50Hz = 20ms
        uint16_t ble_channels[16];
        for(int i = 0; i < 16; i++) {
          if (i < CRSF_NUM_CHANNELS) {
            ble_channels[i] = CRSF_to_US(ChannelData[i]);
          } else {
            ble_channels[i] = 1500; // Failsafe
          }
        }
        EXT_BLE_PublishRC(ble_channels);
        last_ble_rc_update = now;
      }
      #endif
    }
  }
...
void setup()
{
  if (setupHardwareFromOptions())
  {
    ...
    else
    {
      ...
      // Initialize EXTREME-BLE standalone
      DBGLN("Initializing EXTREME-BLE...");
      EXT_BLE_Init();
  #endif
